...
export const supplierFields = (record = {}) => {
  const preferredKeys = [
    'razao_social', 'nome_completo', 'nome_fantasia', 'codigo_fornecedor',
    'razao_social_atu', 'cnpj_cpf_atu',
    'logradouro', 'numero', 'complemento', 'bairro', 'estado', 'cidade', 'cep',
    'cnpj', 'cpf', 'telefone', 'celular', 'inscricao_estadual', 'inscricao_estadual_isento', 'email',
    'website', 'ramo_atividade', 'produtos_servicos', 'condicoes_pagamento',
    'rg', 'cnh_categoria', 'antt', 'responsavel_nome', 'responsavel_cargo',
    'tipo_cadastro', 'nome_solicitante', 'email_solicitante',
    // bancários ficam fora daqui — ver supplierBankFields()
    'banco', 'agencia', 'conta', 'tipo_conta', 'titular_conta', 'cpf_cnpj_titular', 'pix',
  ];
  // Sequência pedida: razão social, fantasia, endereço (rua+nº), bairro, estado (UF),
  // cidade, CEP, CNPJ/CPF, telefone, inscrição estadual (ou isento), e-mail.
  const preferred = [
    ['Razão social / Nome', record.razao_social || record.nome_completo],
    ['Código Protheus', record.codigo_fornecedor],
    ['Nome fantasia', record.nome_fantasia],
    ['Fornecedor a atualizar', record.razao_social_atu],
    ['CNPJ / CPF do fornecedor a atualizar', record.cnpj_cpf_atu],
    ['Endereço', [record.logradouro, record.numero].filter(Boolean).join(', ')],
    ['Complemento', record.complemento],
    ['Bairro', record.bairro],
    ['Estado (UF)', record.estado],
    ...
    ['Condições de pagamento', record.condicoes_pagamento],
...
/* Dados bancários do fornecedor — seção separada de propósito, pra não
   misturar com os dados cadastrais na hora de conferir/copiar pro Protheus. */
export const supplierBankFields = (record = {}) => [
  ['Banco', record.banco],
  ['Agência', record.agencia],
  ['Conta', record.conta],
  ['Tipo de conta', record.tipo_conta],
  ['Titular da conta', record.titular_conta],
  ['CPF/CNPJ do titular', record.cpf_cnpj_titular],
  ['PIX', record.pix],
].filter(([, value]) => isFilled(value));
