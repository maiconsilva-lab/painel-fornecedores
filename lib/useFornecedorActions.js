...
export function useFornecedorActions({ sel, setSel, setShowModal, user, showToast, askConfirm, logAcao, applyRealtimeChange, sendEmail, obsRef }) {
  ...
  const concluirCadastro = async () => {
    ...
    const r = await sendEmail('aprovado', {
      ...
      saudacao: `O cadastro de fornecedor <strong>${sel.razao_social || sel.nome_completo || 'N/A'}</strong> que você solicitou foi concluído com sucesso.`,
      destaque_label: 'CÓDIGO DE FORNECEDOR',
      destaque_valor: concluirData.codigo.trim(),
      rodape_extra: '',
    });
  ...
  const sendDevolutiva = async () => {
    ...
    const r = await sendEmail('devolvido', {
      ...
      saudacao: `O cadastro de fornecedor <strong>${sel.razao_social || sel.nome_completo || 'Cadastro'}</strong> que você enviou precisa de alguns ajustes antes de prosseguir.`,
      destaque_label: 'MOTIVO DA DEVOLUÇÃO',
      destaque_valor: motivoFinal.replace(/\n/g, '<br>'),
      rodape_extra: `Campos a corrigir: ${camposLegiveis}<br><br>Para corrigir, acesse: <a href="${linkCorrecao}">${linkCorrecao}</a>`,
    });
