...
export function useFornecedorActions({ sel, setSel, setShowModal, user, showToast, askConfirm, logAcao, applyRealtimeChange, sendEmail, obsRef }) {
  ...
  const concluirCadastro = async () => {
    ...
    const r = await sendEmail('aprovado', {
      ...
      saudacao: `O cadastro de fornecedor ${sel.razao_social || sel.nome_completo || 'N/A'} que você solicitou foi concluído com sucesso.`,
  ...
  const sendDevolutiva = async () => {
    ...
    const r = await sendEmail('devolvido', {
      ...
      saudacao: `O cadastro de fornecedor ${sel.razao_social || sel.nome_completo || 'Cadastro'} que você enviou precisa de alguns ajustes antes de prosseguir.`,
      destaque_label: 'MOTIVO DA DEVOLUÇÃO',
      destaque_valor: motivoFinal,
      rodape_extra: `Campos a corrigir: ${camposLegiveis}  •  Para corrigir, acesse: ${linkCorrecao}`,
    });
