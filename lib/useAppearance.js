...
export function useAppearance({ user, showToast }) {
  ...
  const [cursorPreset, setCursorPreset] = useState(() => readLS('cursorPreset', 'padrao'));
  ...
  useEffect(() => { writeLS('cursorPreset', cursorPreset); }, [cursorPreset]);
  ...
  const loadPrefs = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.cursor_preset) setCursorPreset(data.cursor_preset);
      }
  ...
  const savePrefs = async (patch) => {
    if (!user) return;
    const payload = { tema, densidade, cursor_preset: cursorPreset, ...patch };
    try {
