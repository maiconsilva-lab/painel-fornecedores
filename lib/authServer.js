...
export async function verifySession(req, roles = null) {
  ...
  const { data, error } = await supa
    ...
    .select('id, nome, email, cargo, telefone, role, ativo, primeiro_login, created_at, session_epoch')
    .eq('id', session.sub)
    .eq('ativo', true)
    .single();

  if (error || !data) return null;
  // session_epoch é incrementado pelo admin no "Forçar logout" — invalida
  // qualquer cookie assinado antes desse momento, mesmo que ainda não tenha
  // expirado.
  if ((session.epoch ?? 0) !== (data.session_epoch ?? 0)) return null;
