...
export function useUsersAdmin({ user, fetchAll, showToast, askConfirm }) {
  ...
  const forcarLogout = (id, nome) => {
    askConfirm('Forçar logout?', `${nome} será desconectado da sessão atual e precisará entrar de novo (a conta continua ativa).`, async () => {
      const res = await fetch(`/api/auth/users/${id}/force-logout`, { method: 'POST', credentials: 'same-origin' });
      const json = await res.json();
      if (!res.ok) { showToast('Erro: ' + (json.error || '')); return; }
      showToast(`${nome} foi desconectado`);
      await fetchAll();
    }, false);
  };
