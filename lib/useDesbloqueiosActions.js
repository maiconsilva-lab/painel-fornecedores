import { mutateTable } from './dataApi';

/* Handlers de desbloqueios, extraídos do app/page.js. Mesmo padrão do
   useProdutosActions — sem state próprio. */
export function useDesbloqueiosActions({ user, showToast, askConfirm, logAcao, applyRealtimeChange, sendEmail }) {
  const pegarDesbloqueio = async (id) => {
    const { row } = await mutateTable(user.id, 'desbloqueios', 'update', { id, data: { atribuido_para: user.nome, status:'em_analise' }, returning: true });
    if (row) applyRealtimeChange('desbloqueios', { eventType:'UPDATE', new: row });
    logAcao('atribuiu', 'desbloqueio', id, { para: user.nome });
  };

  const excluirDesbloqueio = (id) => {
    askConfirm('Excluir pedido?', 'Esta ação não pode ser desfeita.', async () => {
      try {
        await mutateTable(user.id, 'desbloqueios', 'delete', { id });
      } catch (err) { showToast('Erro ao excluir: ' + err.message); return; }
      applyRealtimeChange('desbloqueios', { eventType:'DELETE', old: { id } });
      showToast('Pedido excluído');
      logAcao('excluiu', 'desbloqueio', id);
    });
  };

  const concluirDesbloqueio = async (d) => {
    const upd = {
      status: 'desbloqueado',
      finalizado_por: user.nome,
      data_finalizacao: new Date().toISOString(),
    };
    let row;
    try {
      ({ row } = await mutateTable(user.id, 'desbloqueios', 'update', { id: d.id, data: upd, returning: true }));
    } catch (err) { showToast(`Erro: ${err.message}`); return; }
    applyRealtimeChange('desbloqueios', { eventType:'UPDATE', new: row });

    const r = await sendEmail('desbloqueio', {
      to_name: d.nome_solicitante,
      to_email: d.email_solicitante,
      titulo: 'Produto desbloqueado ✓',
      saudacao: 'O produto abaixo foi desbloqueado com sucesso.',
      destaque_label: 'PRODUTO DESBLOQUEADO',
      destaque_valor: `${d.nome_produto || 'Produto'} — Código: <strong>${d.codigo_produto || '-'}</strong>`,
      rodape_extra: '',
    });
    if (r.ok) showToast('Produto desbloqueado e e-mail enviado!');
    else showToast(`Desbloqueado, mas e-mail falhou: ${r.error}`);

    logAcao('desbloqueou', 'desbloqueio', d.id, {
      codigo: d.codigo_produto,
      email_enviado: r.ok,
    });
  };

  const rejeitarDesbloqueio = async (d, motivo) => {
    if (!motivo || !motivo.trim()) { showToast('Informe o motivo da rejeição'); return; }
    const upd = {
      status: 'rejeitado',
      motivo_rejeicao: motivo.trim(),
      finalizado_por: user.nome,
      data_finalizacao: new Date().toISOString(),
    };
    let row;
    try {
      ({ row } = await mutateTable(user.id, 'desbloqueios', 'update', { id: d.id, data: upd, returning: true }));
    } catch (err) { showToast(`Erro: ${err.message}`); return; }
    applyRealtimeChange('desbloqueios', { eventType:'UPDATE', new: row });
    showToast('Desbloqueio rejeitado');
    logAcao('rejeitou', 'desbloqueio', d.id, { motivo: motivo.trim() });
  };

  /* Conclui vários desbloqueios de uma vez, agrupando por solicitante —
     mesma lógica do concluirProdutosEmLote. */
  const concluirDesbloqueiosEmLote = async (desbloqueiosList) => {
    if (!desbloqueiosList.length) return { ok:false };

    const upds = [];
    for (const d of desbloqueiosList) {
      const upd = {
        status: 'desbloqueado',
        finalizado_por: user.nome,
        data_finalizacao: new Date().toISOString(),
      };
      try {
        const { row } = await mutateTable(user.id, 'desbloqueios', 'update', { id: d.id, data: upd, returning: true });
        applyRealtimeChange('desbloqueios', { eventType:'UPDATE', new: row });
        upds.push({ d, ok:true });
      } catch (err) {
        upds.push({ d, ok:false, error: err.message });
      }
    }

    const aprovados = upds.filter(u => u.ok);
    const porSolicitante = new Map();
    for (const item of aprovados) {
      const email = (item.d.email_solicitante || '').trim();
      if (!email) continue;
      if (!porSolicitante.has(email)) porSolicitante.set(email, { nome: item.d.nome_solicitante || 'Solicitante', itens: [] });
      porSolicitante.get(email).itens.push(item.d);
    }

    let emailsOk = 0, emailsFalha = 0;
    for (const [email, grupo] of porSolicitante) {
      const listaTexto = grupo.itens.map(d => `${d.nome_produto || 'Produto'} — Código: <strong>${d.codigo_produto || '-'}</strong>`).join('<br>');
      const r = await sendEmail('desbloqueios_lote', {
        to_name: grupo.nome,
        to_email: email,
        titulo: `${grupo.itens.length} produtos desbloqueados ✓`,
        saudacao: 'Os produtos abaixo foram desbloqueados com sucesso.',
        destaque_label: 'ITENS DESBLOQUEADOS',
        destaque_valor: listaTexto,
        rodape_extra: '',
      });
      if (r.ok) emailsOk++; else emailsFalha++;
      for (const d of grupo.itens) {
        logAcao('desbloqueou', 'desbloqueio', d.id, { codigo: d.codigo_produto, email_enviado: r.ok, lote: true });
      }
    }

    const falhas = upds.filter(u => !u.ok);
    if (falhas.length) showToast(`${aprovados.length} desbloqueado(s), ${falhas.length} falharam`);
    else showToast(`${aprovados.length} desbloqueado(s) — ${emailsOk} e-mail(s) enviado(s)${emailsFalha ? `, ${emailsFalha} falharam` : ''}`);

    return { ok: true, aprovados: aprovados.length, falhas: falhas.length };
  };

  return { pegarDesbloqueio, excluirDesbloqueio, concluirDesbloqueio, rejeitarDesbloqueio, concluirDesbloqueiosEmLote };
}
